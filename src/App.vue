...
function setupScrollAnimations() {
  ...
  if (classCards.length) {
    ...
    ScrollTrigger.create({
      trigger: classCards[0].parentElement,
      start: "top 85%",
      onEnter: () =>
        gsap.to(classCards, {
          y: 0,
  ...
  gsap.utils.toArray(".gsap-stat-bar").forEach((bar) => {
    ...
    ScrollTrigger.create({
      ...
      start: "top 92%",
      onEnter: () =>
        gsap.to(bar, { width: w + "%", duration: 1.2, ease: "power3.out" }),
    });
  });
}
...
<template>
  ...
  <div
    ...
    :class="isScrolled ? 'px-3 pt-2 md:px-4 md:pt-3' : ''"
  >
    <header
    ...
    >
      <div
        class="flex items-center justify-between px-4 py-2.5 md:px-6 transition-all duration-500"
        :class="isScrolled ? 'py-2 md:py-2.5' : 'py-3 md:py-4'"
      >
        <a
          href="#"
          class="nav-animate flex items-center gap-2 font-extrabold transition-all duration-300"
        ...
        <div class="hidden md:flex items-center gap-4">
          <a
            href="https://api.whatsapp.com/send?phone=60176472510&text=.menu"
            class="nav-animate magnetic-btn rounded-lg bg-gradient-to-r from-purple-600 to-pink-600 px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-purple-500/25 hover:shadow-purple-500/40 transition-all duration-300"
            >Mulai Main</a
          >
        </div>
        <button
  ...
  <!-- HERO -->
  <section
    class="relative overflow-hidden min-h-screen flex items-center justify-center pt-20 pb-10 md:pt-24 md:pb-16"
  >
    <div class="pointer-events-none absolute inset-0">
      <div
        class="parallax-blob absolute left-1/2 top-0 -translate-x-1/2 h-[700px] w-[900px] rounded-full bg-purple-600/10 blur-[150px]"
        data-speed="0.3"
      ></div>
      <div
        class="parallax-blob absolute right-0 top-1/4 h-[300px] w-[300px] rounded-full bg-pink-600/8 blur-[100px]"
        data-speed="0.6"
      ></div>
    </div>
    <div class="relative mx-auto max-w-5xl px-6 text-center">
      <div
        class="hero-badge mx-auto mb-8 inline-flex items-center gap-2 rounded-full border border-purple-500/20 bg-purple-500/10 px-4 py-1.5 text-xs font-medium text-purple-400"
      >
      ...
      >
        ...
        <a
          ...
          class="hero-cta magnetic-btn inline-flex items-center justify-center gap-2 rounded-xl border border-slate-700 w-full sm:w-auto px-6 py-3.5 sm:px-8 sm:py-4 text-sm sm:text-base font-semibold text-slate-300 hover:border-purple-500/50 hover:text-white transition-all"
        >
          📖 Lihat Gameplay
        </a>
      </div>
      <!-- Stats -->
      <div
        class="mx-auto mt-10 md:mt-16 grid max-w-3xl grid-cols-4 gap-4 sm:gap-8"
      >
        <div
          v-for="s in [
            { v: '50K+', l: 'Players' },
        ...
        >
          <div
            class="text-xl font-extrabold text-white sm:text-2xl md:text-3xl"
          >
            {{ s.v }}
          </div>
          <div
            class="mt-0.5 text-[10px] font-medium text-slate-500 sm:text-xs md:text-sm"
          >
            {{ s.l }}
          </div>
        </div>
  ...
  <section id="gameplay" class="relative py-24">
    <div class="mx-auto max-w-7xl px-6">
      ...
      <div
        class="gsap-card-grid mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3"
      >
        <div
          v-for="f in gameplayFeatures"
          :key="f.title"
          class="gsap-card group rounded-2xl border border-purple-900/30 bg-[#0f0d1a]/80 p-6 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:border-purple-500/30 hover:shadow-xl hover:shadow-purple-500/5"
        >
        ...
        <div>
          ...
          <div class="mt-8 space-y-3">
            <div
              v-for="c in commands"
              :key="c.cmd"
              class="gsap-cmd-item flex items-center gap-4 rounded-xl border border-purple-900/30 bg-[#0f0d1a] px-4 py-3 transition-all hover:border-purple-500/30"
            >
              <code
        ...
        <div class="gsap-chat-mockup relative overflow-hidden">
          ...
          >
            ...
            <div class="p-4 space-y-3 text-sm">
              <div class="gsap-chat-msg flex justify-end">
                <div
              ...
              <div class="gsap-chat-msg flex justify-start">
                <div
    ...
    <div class="mx-auto max-w-7xl px-6">
      ...
      <div class="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <div
          v-for="c in classes"
          :key="c.name"
          class="gsap-class-card group rounded-2xl border border-purple-900/30 bg-[#0f0d1a]/80 p-6 text-center transition-all duration-300 hover:-translate-y-2 hover:border-purple-500/30 hover:shadow-xl hover:shadow-purple-500/5"
        >
          <div
          ...
          <div class="mt-5 space-y-3 text-left">
            ...
            >
              ...
              <div class="h-1.5 rounded-full bg-slate-800 overflow-hidden">
                <div
                  class="gsap-stat-bar h-full rounded-full"
                  :class="st.cl"
                  :data-width="st.v"
                  style="width: 0%"
                ></div>
  ...
  <section id="leaderboard" class="relative py-16 md:py-24">
    <div class="mx-auto max-w-4xl px-4 md:px-6">
      <div class="gsap-section-header mx-auto max-w-2xl text-center">
        ...
        >
        <h2
          class="mt-3 text-2xl font-extrabold tracking-tight text-white sm:text-3xl md:text-4xl"
        >
          Top
          <span
      ...
      <div
        class="mt-8 md:mt-12 overflow-hidden rounded-xl md:rounded-2xl border border-purple-900/30 bg-[#0f0d1a]/80 backdrop-blur-sm"
      >
        <div
          class="grid grid-cols-3 sm:grid-cols-5 gap-2 sm:gap-4 border-b border-purple-900/30 px-4 sm:px-6 py-2.5 sm:py-3 text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-500"
        >
          <span>Rank</span><span>Player</span
          ><span class="hidden sm:block">Level</span
          ><span class="hidden sm:block">Class</span
        ...
        >
          <span
            ...
            >{{ p.medal || "#" + p.rank }}</span
          >
          <span class="font-bold text-white text-xs sm:text-sm truncate">{{
            p.name
          }}</span>
          <span class="hidden sm:block text-amber-400 font-bold text-sm"
            >Lv.{{ p.level }}</span
          >
          <span class="hidden sm:block text-purple-400 text-sm">{{
            p.cls
          }}</span>
          <span
            class="text-right text-xs sm:text-sm font-bold text-emerald-400"
            >{{ p.power }}</span
          >
        </div>
      </div>
  ...
  <section class="relative py-24">
    <div class="mx-auto max-w-4xl px-6 text-center">
      <div
        class="gsap-cta-box relative overflow-hidden rounded-3xl border border-purple-900/30 bg-[#0f0d1a]/80 px-8 py-16 backdrop-blur-sm sm:px-16"
      >
        <div class="pointer-events-none absolute inset-0">
          <div
            class="parallax-blob absolute left-1/2 top-0 -translate-x-1/2 h-[300px] w-[500px] rounded-full bg-purple-500/10 blur-[100px]"
            data-speed="0.4"
          ></div>
        </div>
        <div class="relative">
          ...
          <a
            href="https://api.whatsapp.com/send?phone=60176472510&text=.menu"
            class="magnetic-btn mt-8 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 px-8 py-4 text-base font-semibold text-white shadow-xl shadow-purple-500/25 hover:shadow-purple-500/40 transition-all"
          >
            🚀 Mulai Main Sekarang
          </a>
        </div>
    ...
    <div class="mx-auto max-w-7xl px-6 py-12">
      ...
      <div class="mt-8 border-t border-purple-900/20 pt-6 text-center">
        <p class="text-sm text-slate-700">
          &copy; 2025 Yurizaki RPG. All rights reserved. Made with ❤️
        </p>
      </div>
    </div>
  </footer>
